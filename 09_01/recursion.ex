defmodule Recursion do
  ...
  @doc """
  Accept an integer.
  Add 1 to the integer and bind the result to the variable `sum`.
  Output `sum` using IO.puts/2.
  Send `sum` to infinty/1 to repeat the process infinitely.
  """
  def infinity(i) when is_integer(i) do
    sum = i + 1
    IO.puts(sum)
    infinity(sum)
  end
  ...
  @doc """
  Accept an integer and a counter representing the number of times to repeat the process.
  ...
  When the counter reaches 1, output the final sum but do not send it to add_one/2 any more.
  """
